...
$(document).ready(function () {
    ...
    $("#activitySearch").keyup(function(){
        var logDetailsActivitySearch = JSON.parse(localStorage.getItem("logActivity"));
        var searchInput = $("#activitySearch").val();
        for (let logSearch = 0; logSearch < logDetailsActivitySearch.length; logSearch++) {
            if(searchInput === logDetailsActivitySearch[logSearch].log_type){
                searchResult.push(logDetailsActivitySearch[logSearch]);
                console.log(searchResult);
                $("#activityLogMainBlock").hide();
                $("#activityLogMainBlockSearch").show();
                var searchLog = `
                    <div class="timeLineContent">
                    <span class="timeLineImage">    
                        <img src="/images/profile.jpg" width="30px" height="30px" alt="timeline" class="timeLineImageSrc">
                    </span>
                    <span class="timeLineName">${logDetailsActivitySearch[logSearch].log_userName}</span>
                    <span class="timeLineActivity">${logDetailsActivitySearch[logSearch].log_activity}</span>
                    <span class="timeLineImage">
                        <img src="/images/timeline.png" alt="timelineTime" class="timeLineImageSrcTime">
                    </span>
                    <span class="timeLineActivityTime">${logDetailsActivitySearch[logSearch].log_time}</span>
                    </div>
                    `;
                    $("#activityLogMainBlockSearch").append(searchLog);
            }
            else if(searchInput==="")
            {   
                $("#activityLogMainBlockSearch").html = "";
                $("#activityLogMainBlock").show();
                $("#activityLogMainBlockSearch").hide();
                searchResult = [];
            }
        }
    });
